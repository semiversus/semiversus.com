...
class AdmonitionProcessor(markdown.blockprocessors.BlockProcessor):
    ...
    def run(self, parent, blocks):
        ...
        if m:
            ...
            div = markdown.util.etree.SubElement(parent, 'div')
            div.set('class', '%s %s' % (self.CLASSNAME, klass))
            if title:
                p = markdown.util.etree.SubElement(div, 'div')
                p.text = title
        ...
        if block:
          div=markdown.util.etree.SubElement(div, 'div')
          div.set('class', 'panel-body')
